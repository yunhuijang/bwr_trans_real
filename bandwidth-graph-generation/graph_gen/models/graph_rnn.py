...
class GraphRNNSimple(BaseLightningModule):
    ...
    def get_loss(
        ...
        self.log(
            f"{prefix}/auprc",
            binary_average_precision(sigmoid_adj_pred, packed_output.data.long()),
            prog_bar=True,
        )
        self.log(
            f"{prefix}/precision",
    ...
    def test_step(self, batch, _=None):
        ...
        # batchwise AUPRC
        batch_auprc = binary_average_precision(sigmoid_adj_pred, actual.long())
        # per graph AUPRC
        ...
        per_graph_auprc = torch.tensor([
            binary_average_precision(prob, act.long())
    ...
    def test_epoch_end(self, outputs):
        ...
        self.log("test/auprc", binary_average_precision(probs, actual.long()))
        # per graph auprc
        self.log(
    ...
    def evaluate_temperature(
        self, temp: float, real_graphs: list,
        max_graph_len: int, samples, data_name, ts
    ) -> dict:
        ...
        if not sampled_graphs:
            return {}
        save_graph_list(data_name, ts, sampled_graphs)
        plot_dir = f'{data_name}/{ts}'
    ...
    def find_best_temperature(
        self, real_graphs: list, max_graph_len: int,
    ...
    def gen_samples(self, real_graphs, max_graph_len, temperature):
        samples = self.sample(len(real_graphs), steps=max_graph_len, temperature=temperature)
        return samples